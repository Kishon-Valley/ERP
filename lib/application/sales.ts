...
export async function postSale(input: CheckoutInput) {
  ...
  for (const line of input.lines) {
    ...
    taxTotal += tax;
    resolved.push({ productId: line.productId, uom: line.uom, quantity: line.quantity, unitPrice, discountRate, taxRate: Number(product.tax_rate), lineTotal: net + tax });
  }
  ...
  if (exposure + total > Number(customer[0].credit_limit)) {
    throw new DomainError("CREDIT_LIMIT_EXCEEDED", "Sale would exceed the customer's available credit.", {
      creditLimit: Number(customer[0].credit_limit), currentExposure: exposure, requested: total
    });
  }
  ...
  await reserveStock(input.organizationId, order[0].id, input.lines.map(line => ({ ...line, warehouseId: input.warehouseId })));
